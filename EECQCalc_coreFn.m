function [EECQout,Ceqout,RLM,SNRdB,BER] = EECQCalc_coreFn(Vin,SSPRQseq,samplesPerUI,VRMS,BERType)
%% Initialization
...
% y=VinFilteredReshaped(:,i1)/(Vpp5Percent) + VRMS*randn(size(VinFilteredReshaped(:,i1)));
% x=inputVolt(:,i1);
% Y = hankel(y(1:nTaps),circshift(y.',-(nTaps-1)));
% X = hankel(x(1:nTaps),circshift(x.',-(nTaps-1)));
% C=Y*(Y');
% T=X*(Y');
% W=(T/C);
% Xhat=W*Y;
% NMSE=mean(abs(X-Xhat).^2,2)./mean(abs(X).^2,2);
% [NMSEValueTmp,NMSEInd]=min(NMSE);
% SNRdB=pow2db(2./(NMSE(NMSEInd-ell)+NMSE(NMSEInd+ell)));
% uniqueSyms=sort(unique(x),'ascend');
% th=(uniqueSyms(1:end-1)+uniqueSyms(2:end))/2;
% 
% xhatL=Xhat(NMSEInd-ell,:).';
% xhatR=Xhat(NMSEInd+ell,:).';
% 
% xhatL=circshift(xhatL,NMSEInd-ell-1);
% xhatR=circshift(xhatR,NMSEInd+ell-1);
% ind=x==uniqueSyms(1);
% BER(1)=sum(xhatL(ind)>th(1))/(4*length(x)) + sum(xhatR(ind)>th(1))/(4*length(x));
% for k=2:length(th)
%     ind=x==uniqueSyms(k);
% BER(k)=sum(xhatL(ind)>th(k) | xhatL(ind)<th(k-1))/(4*length(x)) + sum(xhatR(ind)>th(k) | xhatR(ind)<th(k-1))/(4*length(x));
% end
% ind=x==uniqueSyms(end);
% BER(end+1)=sum(xhatL(ind)<th(end))/(4*length(x)) + sum(xhatR(ind)<th(end))/(4*length(x));
